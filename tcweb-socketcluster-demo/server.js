...
//引入ProducerMgr
var ProducerMgrLocal = require('./business_ext/ProducerMgrLocal.js');

//引入ProducerMgrRemote  dataSource来自远程socket推送
var ProducerMgrRemote = require('./business_ext/ProducerMgrRemote.js');

//引入SCMonitor
var SCMonitor = require('./business_ext/SCMonitor.js');
...
var start = function() {
    ...
    //初始化SCMonitor
    let scMonitor = new SCMonitor(socketCluster, {});
    //启动SCMonitor
    scMonitor.init();

    //远程数据是否采用socket推送，有DS_SOCKETCLUSTER_PORT就是通过ws从远程server获取数据
    let dataSourceServerPort = process.env.DS_SOCKETCLUSTER_PORT || 8000;
    let dataSourceServerIp = process.env.DS_SOCKETCLUSTER_IP || '127.0.0.1';

    let producerMgr = null;
    if (dataSourceServerPort){
        //初始化ProducerMgr
        producerMgr = new ProducerMgrRemote(socketCluster, {
            dir: __dirname + '/producer_remote',
            dataSourceServerPort: dataSourceServerPort,
            dataSourceServerIp: dataSourceServerIp
        });
    } else {
        producerMgr = new ProducerMgrLocal(socketCluster, {
            dir: __dirname + '/producer'
        });
    }
    //启动ProducerMgr
    producerMgr.init();
